...
class SleepLogMapperTest {
    ...
    @Test
    fun sleepLog_allSyncStatusValues_roundTrip() {
        SyncStatus.entries.forEach { status ->
            ...
            assertEquals(status, log.toEntity().toDomain().syncStatus)
        }
    }

    @Test
    fun sleepLog_toEntity_setsCorrectFields() {
        val log = SleepLog(
            id = "log-3",
            date = baseDate,
            wakeTime = baseInstant,
            cycleCount = 4,
            cycleDurationUsed = 95,
            sleepLatencyUsed = 20,
            createdAt = baseInstant,
            updatedAt = baseInstant,
        )
        ...
        assertEquals("log-3", entity.id)
        assertEquals(baseDate, entity.date)
        assertEquals(baseInstant, entity.wakeTime)
        assertEquals(4, entity.cycleCount)
        assertEquals(95, entity.cycleDurationUsed)
        assertEquals(20, entity.sleepLatencyUsed)
    }
}
